...
_e(function (E, $) {
    ...
    E.plugin(function () {
        ...
        var $anchor = $('<a href="#"><i class="wangeditor-menu-img-omega"></i></a>');
        var $setTitleAndAlt = $('<a href="#"><i class="wangeditor-menu-img-wrench"></i></a>');
        ...
        var $anchorInputContainer = $('<div style="display:none;"></div>');
        var $anchorInput = $('<input type="text" style="height:26px; margin-left:10px; width:200px;" placeholder="锚点名称，提交空值则删除锚点"/>');
        var $anchorBtnSubmit = $('<button class="right">' + lang.submit + '</button>');
        var $anchorBtnCancel = $('<button class="right gray">' + lang.cancel + '</button>');

        var $imgDescInputContainer = $('<div style="display:none;"></div>');
        var $imgTitleInputRow = $('<div style="width:300px;"><label style="width:15%; text-align:right; display:inline-block">标题：</label></div>');
        var $imgTitleInput = $('<input type="text" style="height:30px; margin-left:10px; width:80%;" placeholder="图片标题，提交空值则删除标题"/>');
        var $imgAltInputRow = $('<div style="width:300px;"><label style="width:15%; text-align:right; display:inline-block">Alt：</label></div>');
        var $imgAltInput = $('<input type="text" style="height:30px; margin-left:10px; width:80%;" placeholder="图片Alt，提交空值则删除Alt"/>');
        var $imgDescBtnSubmit = $('<button class="right">' + lang.submit + '</button>');
        var $imgDescBtnCancel = $('<button class="right gray">' + lang.cancel + '</button>');
        ...
        function imgAnchor(e, anchor) {
            ...
            if (anchor == null) {
                // anchor 无值，是获取anchor
                return $currentImg.attr('id') || '';
            } else {
                if (anchor) {
                    commandFn = function () {
                        $currentImg.attr('id', anchor);
                    };
                } else {
                    commandFn = function () {
        ...
        function imgTitle(e, title) {
            ...
            if (title == null) {
                // title 无值，是获取title
                return $currentImg.attr('title') || '';
            } else {
                if (title) {
                    commandFn = function () {
                        $currentImg.attr('title', title);
                    };
                } else {
                    commandFn = function () {
                        $currentImg.removeAttr('title');
                    };
                }
            }
        ...
        function imgAlt(e, alt) {
            ...
            if (alt == null) {
                // alt 无值，是获取alt
                return $currentImg.attr('alt') || '';
            } else {
                if (alt) {
                    commandFn = function () {
                        $currentImg.attr('alt', alt);
                    };
                } else {
                    commandFn = function () {
                        $currentImg.removeAttr('alt');
                    };
                }
            }
        ...
        function render() {
            ...
            $anchorInputContainer.append($anchorInput)
                               .append($anchorBtnCancel)
                               .append($anchorBtnSubmit);

            $imgDescInputContainer.append($imgTitleInputRow.append($imgTitleInput))
                                  .append($imgAltInputRow.append($imgAltInput))
                                  .append($imgDescBtnCancel)
                                  .append($imgDescBtnSubmit);

            // 拼接 渲染到页面上
            $toolbar.append($triangle)
                    .append($menuContainer)
                    .append($linkInputContainer)
                    .append($anchorInputContainer)
                    .append($imgDescInputContainer);
        ...
        function bindToolbarEvent() {
            ...
            $anchor.click(function (e) {
               e.preventDefault();
                ...
                // 获取当前锚点，并显示
                $anchorInput.val(imgAnchor(e));

                $menuContainer.hide();
                $anchorInputContainer.show();
            });
            // 设置锚点
            $anchorBtnSubmit.click(function (e) {
                e.preventDefault();
            ...
            // 取消设置锚点
            $anchorBtnCancel.click(function (e) {
                e.preventDefault();

                $menuContainer.show();
                $anchorInputContainer.hide();
            });

            $setTitleAndAlt.click(function (e) {
                e.preventDefault();

                // 获取当前Title，并显示
                $imgTitleInput.val(imgTitle(e));
                // 获取当前Alt，并显示
                $imgAltInput.val(imgAlt(e));

                $menuContainer.hide();
                $imgDescInputContainer.show();
            });
            // 设置描述
            $imgDescBtnSubmit.click(function (e) {
                e.preventDefault();

                var title = $.trim($imgTitleInput.val());
                imgTitle(e, title);

                var alt = $.trim($imgAltInput.val());
                imgAlt(e, alt);
            ...
            // 取消设置描述
            $imgDescBtnCancel.click(function (e) {
                e.preventDefault();

                $menuContainer.show();
                $imgDescInputContainer.hide();
            });
        }
